...
def generate(generated_file_name="ave_pool_acc_innerdf.h"):
	str1 = "#ifndef _AVE_POOL_ACC_H_" + EOL
	str1 += "#define _AVE_POOL_ACC_H_" + EOL + EOL 
	...
	str1 += "class ave_pool_acc {" + EOL + EOL
	str1 += "private:" + EOL
	str1 += "	int pool_layer_number;" + EOL + EOL
	str1 += "public:" + EOL
	str1 += "	ave_pool_acc() : pool_layer_number(0) {pool_layer_number = 0;};" + EOL + EOL
	...
	str1 += "    // Ave pooling computation kernel" + EOL
	...
	str1 += "    void ave_pool_layer_acc(" + EOL
	...
	str1 += "        int TR=0;" + EOL
	str1 += "        int TC=0;" + EOL
	str1 += "        T in_buf[Tn][(Tr-1)*S_max + K_max][(Tc-1)*S_max + K_max];" + EOL
	str1 += "        G out_buf[Tn][Tr][Tc];" + EOL + EOL
	str1 += "#if _HLS_MODE_" + EOL
	str1 += "#pragma HLS ARRAY_PARTITION variable=in_buf complete dim=1" + EOL
	str1 += "#pragma HLS ARRAY_PARTITION variable=out_buf complete dim=1" + EOL
	str1 += "#endif" + EOL + EOL
	str1 += "#if _C_DEBUG_MODE_" + EOL
	str1 += "#if _KERNEL_DEBUG_" + EOL
	str1 += '            cout << "Starting ave_pool_acc_innerdf layer ...." << endl;' + EOL
	str1 += "            //buffer local data initiallization: must do it in C++ debug!" + EOL
	str1 += "            out_buf_reset(out_buf);" + EOL
	str1 += "#endif" + EOL
	str1 += "#endif" + EOL
	...
	str1 += "                        // load input data" + EOL
	str1 += "                        in_buf_load(in_buf"
	for j in range(1,port_num + 1):
		str1 += ", in_data_" + str(j)

	str1 += ", n, r, c, S, K, P, R, C, N, R_IN, C_IN, TR, TC);" + EOL
	str1 += "  //------------------------------compute buffered data -----------------------------------//" + EOL
	str1 += "                        pool_engine(in_buf, out_buf, S, n, r, c, K, R, C, TR, TC);" + EOL
	str1 += "  //---------------------------transfer output data----------------------------------------//" + EOL
	str1 += "                        // transfer output data" + EOL
	str1 += "                        output_res(out_buf"
	for j in range(1,port_num + 1):
	...
	for j in range(1,port_num + 1):
		...
		str1 += "                        ave_pool_out << *(out_data_" + str(j) + ' + i*R*C + j*C + k) << " ";' + EOL
		str1 += "                    }ave_pool_out << endl;" + EOL
		str1 += "                }ave_pool_out << endl;" + EOL

	str1 += "            }ave_pool_out.close();" + EOL
	str1 += "#endif" + EOL
	str1 += "#endif" + EOL
	str1 += "    }" + EOL
	str1 += "};" + EOL
