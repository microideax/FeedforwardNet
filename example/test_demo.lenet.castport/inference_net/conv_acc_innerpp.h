...
#pragma HLS UNROLL
    ...
    void output_res_512(G out_buf[][Tr][Tc],
                        Itf *out_data,
                        int          out_offset,
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
        cout << "CONV ACC: Starting conv acc single buffer test ...." << endl;
...
/*
#if _C_DEBUG_MODE_
int n_size = Tn<N?Tn:N;
int m_size = Tm<M?Tm:M;
int r_size = Tr<R_OUT?Tr:R_OUT;
//int r_size = 32;
int c_size = Tc<C_OUT?Tc:C_OUT;
//int c_size = 32;
cout << "CONV ACC: Input matrix size: <" << n_size << "," << (Tr - 1) * S_max + K_max << "," << (Tr - 1) * S_max + K_max << ">" << endl;
...
cout << endl;
cout << "CONV ACC: Input layer bias data: <" << Tm << ">" << endl;
for (int i = 0; i < Tm && i < M; i++){
    cout << setw(3) << b_buf_0[i] << " ";
}
cout << endl;
cout << "CONV ACC: Input layer weight data: " << (Tn<N?Tn:N)*(Tm<M?Tm:M) << endl;
for(int i = 0; i < Tn && i < N; i++){
    for(int j=0; j < Tm && j < M; j++){
        for(int k1 = 0; k1 < K; k1++){
            for(int k2 = 0; k2 < K; k2++){
                cout << setw(3) << w_buf_0[i][j][k1][k2] << " ";
            }
            cout << endl;
        }
        cout << endl;
...
//int c_size = Tc<C_OUT?Tc:C_OUT;
//int c_size = 32;
cout << "CONV ACC: out_buf_0 matrix size: <" << m_size << "," << r_size << "," << c_size << ">" << endl;
for(int i = 0; i < m_size; i++){
    for(int j=0; j < r_size; j++){
        for(int k=0; k < c_size; k++){
            cout << setw(3) << out_buf_0[i][j][k] << " ";
        }
        cout << endl;
    }
    cout << endl;
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
        cout << "CONV ACC: Finished conv_acc_single buffer test ...." << endl;
        /*
        ofstream conv_out;
