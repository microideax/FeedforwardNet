...
#ifndef _CONV_MAX_POOL_ACC_H_
#define _CONV_MAX_POOL_ACC_H_
...
class conv_max_pool_acc {

private:
    int conv_layer_number;

public:
    conv_max_pool_acc() : conv_layer_number(0) {conv_layer_number = 0;};

///////////////////////------------------conv accelerator----------------//////////////////////////
    void conv_max_pool_layer_acc(
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Starting conv_max_pool_acc layer ...." << endl;
...
#pragma HLS UNROLL
                        ...
                        // max pooling & transfer output data
                            ...
                            for(int tr=0; tr < Tr; tr+=S_P){
                                ...
                                for(int tc=0; tc < Tc; tc+=S_P){
                                    ...
                                    if((tc + j)>=TC){
                                        ...
                                        if(i==0&&j==0){
                                            out_buf_temp[tm][tr/S_P][tc/S_P] = relu(out_buf[tm][tr][tc]);
                                        }else{
                                            out_buf_temp[tm][tr/S_P][tc/S_P] = (out_buf_temp[tm][tr/S_P][tc/S_P] > (relu(out_buf[tm][tr+i][tc+j])) ? out_buf_temp[tm][tr/S_P][tc/S_P] : (relu(out_buf[tm][tr+i][tc+j])));
                                        }
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Finished conv_max_pool_acc layer ...." << endl;
            cout << endl;
            ofstream conv_out;
            conv_out.open("conv_max_pool_out_data.txt", ios::app);
            conv_out <<"conv_max_pool output: "<< endl;
            for (int i = 0; i < M; i++) {
                for (int j = 0; j < R_P; j++) {
                    for(int k = 0; k < C_P; k++){
