...
#include "conv_acc_innerdf.h"
#include "max_pool_acc_innerdf.h"
#include "config.h"

conv_acc<data_type, data_type_w, data_type_o, 16, 4, 13, 13, 6, 11> convAcc1;//{0<Tm<=M;0<Tn<=N;0<Tr<=R;0<Tc<=C;}
...
void conv_layer_new(
    ...
    convAcc1.conv_layer_acc(N, K, M, R, C, S, P, act, in_data, layer_weights, layer_bias, out_data, weight_offset, bias_offset, in_offset, out_offset);
...
void max_pool_layer_new(
    ...
    maxPoolAcc1.max_pool_layer_acc(R_in, C_in, N, K, R, C, S, P, act, in_data, out_data);
