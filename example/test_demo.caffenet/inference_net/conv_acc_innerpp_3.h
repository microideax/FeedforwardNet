#ifndef _CONV_ACC_3_H_
#define _CONV_ACC_3_H_
...
public:
    conv_acc_3() : conv_layer_number(0) { conv_layer_number = 0; };
...
#pragma HLS PIPELINE
                for (int i = 0; i < Tn && i < N; i += 7) {
                    ...
                    if ((n + Tn > N && (i + 0) % 8 >= N - n) || j < 0 || j >= R_IN || k < 0 || k >= C_IN) {
                        ...
                        buf[i + 0][j - r * S + P][k - c * S + P] = *(i_data_0 + in_offset + (i + 0 + n) / 8 * R_IN * C_IN + j * C_IN + k);
                        buf[i + 1][j - r * S + P][k - c * S + P] = *(i_data_1 + in_offset + (i + 1 + n) / 8 * R_IN * C_IN + j * C_IN + k);
                        buf[i + 2][j - r * S + P][k - c * S + P] = *(i_data_2 + in_offset + (i + 2 + n) / 8 * R_IN * C_IN + j * C_IN + k);
                        buf[i + 3][j - r * S + P][k - c * S + P] = *(i_data_3 + in_offset +
                                                                     (i + 3 + n) / 8 * R_IN * C_IN +
                                                                     j * C_IN + k);
                        buf[i + 4][j - r * S + P][k - c * S + P] = *(i_data_4 + in_offset +
