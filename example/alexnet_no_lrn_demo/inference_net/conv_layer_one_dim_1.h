            ...
            for (int j = _CONV_KERNEL_SIZE_ / 2 - _CONV_PADDING_; j < _INPUT_SIZE_ + _CONV_PADDING_ - _CONV_KERNEL_SIZE_ / 2; j += _CONV_STRIDE_) {
                ...
                for (int a = (-_CONV_KERNEL_SIZE_ / 2); a <= _CONV_KERNEL_SIZE_ / 2; a++) {
                    for (int b = (-_CONV_KERNEL_SIZE_ / 2); b <= _CONV_KERNEL_SIZE_ / 2; b++) {
                        if ((i + a >= 0) && (i + a < _INPUT_SIZE_) && (j + b >= 0) && (j + b < _INPUT_SIZE_)) {//if overlapped
                            d_buf[(a + _CONV_KERNEL_SIZE_ / 2) * _CONV_KERNEL_SIZE_ + (b + _CONV_KERNEL_SIZE_ / 2)] = *(in_data + _INPUT_SIZE_ * (i + a) + (j + b));
                        }
                        else
...
#pragma HLS UNROLL
                        T data = *(d_buf + (ii + _CONV_KERNEL_SIZE_ / 2) * _CONV_KERNEL_SIZE_ + (jj + _CONV_KERNEL_SIZE_ / 2));
                        W weight = *(w_buf + (ii + _CONV_KERNEL_SIZE_ / 2) * _CONV_KERNEL_SIZE_ + (jj + _CONV_KERNEL_SIZE_ / 2));
                        out_temp += data * weight;
                    }
                }
                *(out_data + ((_INPUT_SIZE_ + _CONV_PADDING_ * 2 - _CONV_KERNEL_SIZE_) / _CONV_STRIDE_ + 1) * ((i - _CONV_KERNEL_SIZE_ / 2 + _CONV_PADDING_) / _CONV_STRIDE_) + (
                            (j - _CONV_KERNEL_SIZE_ / 2 + _CONV_PADDING_) / _CONV_STRIDE_)) = out_temp;
                }
            }
