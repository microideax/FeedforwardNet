...
#pragma HLS INTERFACE m_axi depth=50  port=fc_8_out_a
	...
	C3.conv_layer_a(activation_type, output_temp_2, conv_weight_port+288*3*3+12288*3*3, conv_bias_port+96+256, output_temp_1);
        for(int addr = 0; addr < 96*16*16; addr++){
        	output_temp_2[addr] = data_type_o(0);
    	}
	C4.conv_layer_a(activation_type, output_temp_1, conv_weight_port+288*3*3+12288*3*3+98304*3*3, conv_bias_port+96+256+384, output_temp_2);
    	for(int addr = 0; addr < 96*16*16; addr++){
        	output_temp_1[addr] = data_type_o(0);
    	}
	C5.conv_layer_a(activation_type, output_temp_2, conv_weight_port+288*3*3+12288*3*3+98304*3*3+73728*3*3, conv_bias_port+96+256+384+384, output_temp_1);
    	for(int addr = 0; addr < 96*16*16; addr++){
        	output_temp_2[addr] = data_type_o(0);
    	}
