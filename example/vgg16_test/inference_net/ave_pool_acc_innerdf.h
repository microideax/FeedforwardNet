...
public:
    ...
    // Load input data
    void in_buf_load(T buf[][(Tr-1)*S_max + K_max][(Tc-1)*S_max + K_max],T *in_data_1,T *in_data_2, int n, int r, int c, int S, int K, int P, int R, int C, int N, int R_IN, int C_IN, int TR, int TC) {
        for (int i = n; i < n + Tn; i+=2){
                ...
                for (int k = c * S - P; k < c * S + TC - P; k++) {
                    if (j < 0 || j >= (R_IN - 2 * P) || k < 0 || k >= (C_IN - 2 * P)) {
                        ...
                        if (N < n + Tn && i + 0 >= N) {
                            ...
                            buf[i + 0 - n][j - r * S + P][k - c * S + P] = *(in_data_1 + i/2 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                        }
                        if (N < n + Tn && i + 1 >= N) {
                            buf[i + 1 - n][j - r * S + P][k - c * S + P] = T(0);
                        }else {
                            buf[i + 1 - n][j - r * S + P][k - c * S + P] = *(in_data_2 + i/2 * (R_IN - 2 * P) * (C_IN - 2 * P) + j * (C_IN - 2 * P) + k);
                        }
                   }
               }
			}
...
#pragma HLS UNROLL
    ...
    void output_res(G out_buf[][Tr][Tc],G *out_data_1,G *out_data_2, int n, int r, int c, int N, int R, int C, bool act){
            for (int i = n; i < n + Tn; i += 2) {
                if (N < n + Tn && i == N) { break; }
                for (int j = r; j < r + Tr; j++) {
                    ...
                    for (int k = c; k < c + Tc; k++) {
                        ...
                        if (act) {
                        	if (i + 0 <N)
                            	*(out_data_1 + (i/2) * R * C + j * C + k) = relu(out_buf[i + 0 - n][j - r][k - c]);
                        	if (i + 1 <N)
                            	*(out_data_2 + (i/2) * R * C + j * C + k) = relu(out_buf[i + 1 - n][j - r][k - c]);
                        }
                        else {
                        	if (i + 0 <N)
                            	*(out_data_1+ (i/2) * R * C + j * C + k) = out_buf[i + 0 - n][j - r][k - c];
                        	if (i + 1 <N)
                            	*(out_data_2+ (i/2) * R * C + j * C + k) = out_buf[i + 1 - n][j - r][k - c];
                        }
                    }
                }
            }
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Starting ave_pool_acc_innerdf layer ...." << endl;
...
#pragma HLS DATAFLOW
   ...
   //--------------------------Load input B W D in ping-pong manner-------------------------//
                        // load input data
                        in_buf_load(in_buf, in_data_1, in_data_2, n, r, c, S, K, P, R, C, N, R_IN, C_IN, TR, TC);
  //------------------------------compute buffered data -----------------------------------//
                        pool_engine(in_buf, out_buf, S, n, r, c, K, R, C, TR, TC);
  //---------------------------transfer output data----------------------------------------//
                        // transfer output data
                        output_res(out_buf, out_data_1, out_data_2, n, r, c, N, R, C, act);
                    }
            }
        }
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Finished ave_pool_acc_innerdf layer ...." << endl;
            ofstream ave_pool_out;
            ave_pool_out.open("ave_pool_out_data.txt", ios::app);
            ave_pool_out <<"ave_pool output: "<< endl;
            for (int i = 0; i < N/2; i++) {
                for (int j = 0; j < R; j++) {
                    for(int k = 0; k < C; k++){
                        ave_pool_out << *(out_data_1 + i*R*C + j*C + k) << " ";
                    }ave_pool_out << endl;
                }ave_pool_out << endl;
                for (int j = 0; j < R; j++) {
                    for(int k = 0; k < C; k++){
                        ave_pool_out << *(out_data_2 + i*R*C + j*C + k) << " ";
                    }ave_pool_out << endl;
                }ave_pool_out << endl;
            }ave_pool_out.close();
#endif
