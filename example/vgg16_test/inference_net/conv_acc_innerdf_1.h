...
#pragma HLS UNROLL
    ...
    // Load input data
    void in_buf_load(T buf[][(Tr-1)*S_max + K_max][(Tc-1)*S_max + K_max],T *in_data_1,T *in_data_2, int in_offset, int n, int r, int c, int S, int K, int P, int R_IN, int C_IN, int N) {
                ...
                for (int k = c * S - P; k < (c + Tc - 1) * S + K - P; k++) {
                    if (j < 0 || j >= R_IN || k < 0 || k >= C_IN) {
                        ...
                        if (N < n + Tn && i + 0 >= N) {
                            ...
                            buf[i + 0 - n][j - r * S + P][k - c * S + P] = *(in_data_1 + in_offset + i/2 * R_IN * C_IN + j * C_IN + k);
                        }
                        if (N < n + Tn && i + 1 >= N) {
                            buf[i + 1 - n][j - r * S + P][k - c * S + P] = T(0);
                        }else {
                            buf[i + 1 - n][j - r * S + P][k - c * S + P] = *(in_data_2 + in_offset + i/2 * R_IN * C_IN + j * C_IN + k);
                        }
                   }
               }
			}
...
#pragma HLS UNROLL
    ...
    void output_res(G out_buf[][Tr][Tc],G *out_data_1,G *out_data_2, int out_offset, int n, int m, int r, int c, int N, int M, int R_OUT, int C_OUT, bool act
#if _BATCH_NORM_
            ...
            for (int i = m; i < m + Tm; i += 2) {
                ...
                for (int j = r; j < r + Tr; j++) {
                    ...
                    for (int k = c; k < c + Tc; k++) {
                        ...
                        if (act) {
                        	if (i + 0 <M)
                            	*(out_data_1 + out_offset + (i/2) * R_OUT * C_OUT + j * C_OUT + k) = relu(out_buf[i + 0 - m][j - r][k - c]);
                        	if (i + 1 <M)
                            	*(out_data_2 + out_offset + (i/2) * R_OUT * C_OUT + j * C_OUT + k) = relu(out_buf[i + 1 - m][j - r][k - c]);
                        }
                        else {
                        	if (i + 0 <M)
                            	*(out_data_1 + out_offset + (i/2) * R_OUT * C_OUT + j * C_OUT + k) = out_buf[i + 0 - m][j - r][k - c];
                        	if (i + 1 <M)
                            	*(out_data_2 + out_offset + (i/2) * R_OUT * C_OUT + j * C_OUT + k) = out_buf[i + 1 - m][j - r][k - c];
                        }
                    }
                }
            }
...
#pragma HLS DATAFLOW
   ...
   //--------------------------Load input B W D in ping-pong manner-------------------------//
                        ...
                        // load input data
                        in_buf_load(in_buf_1, in_data_1, in_data_2, in_offset, n, r, c, S, K, P, R_IN, C_IN, N);
                        // load input weights
  ...
  //---------------------------transfer output data----------------------------------------//
                        // transfer output data
                        output_res(out_buf_1, out_data_1, out_data_2, out_offset, n, m, r, c, N, M, R_OUT, C_OUT, act 
#if _BATCH_NORM_
