...
class conv_layer_buf {

private:
    int conv_layer_number;
    int out_data_size;
public:
    conv_layer_buf() : conv_layer_number(0) { out_data_size = (_INPUT_SIZE_ + _CONV_PADDING_ * 2 - _CONV_KERNEL_SIZE_) / _CONV_STRIDE_ + 1;
    };

/************************************************************************************************/
    void conv_kernel_input_buf(
            T in_data[_INPUT_SIZE_][_INPUT_SIZE_],
            W kernel_weights[_CONV_KERNEL_SIZE_][_CONV_KERNEL_SIZE_],
            G out_data[BUF_1_Y_DIM][BUF_1_Z_DIM]) {
...
#if _C_DEBUG_MODE_
        #if _KERNEL_DEBUG_
        cout << "Starting convolution input layer with buf...." << endl;
#endif
#endif
        ...
        for (int c = 0; c < _GROUP_; c++) {//group loop
            for (int b = c * _OUT_CHANNEL_NUM_ / _GROUP_; b < c * _OUT_CHANNEL_NUM_ / _GROUP_ + _OUT_CHANNEL_NUM_ / _GROUP_; b++) {//output kernel loop
                for (int a = c * _IN_CHANNEL_NUM_ / _GROUP_; a < c * _IN_CHANNEL_NUM_ / _GROUP_ + _IN_CHANNEL_NUM_ / _GROUP_; a++) {//input kernel loop
                    conv_kernel_input_buf(in_data3D[a],
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
        cout << "Finished convolution input layer with buf...." << endl;
        ofstream out_conv_a;
...
/************************************************************************************************/
    ...
    void conv_layer_general_buf(
        ...
        for (int c = 0; c < _GROUP_; c++) {//group loop
            for (int b = c * _OUT_CHANNEL_NUM_ / _GROUP_; b < c * _OUT_CHANNEL_NUM_ / _GROUP_ + _OUT_CHANNEL_NUM_ / _GROUP_; b++) {//output kernel loop
                for (int a = c * _IN_CHANNEL_NUM_ / _GROUP_; a < c * _IN_CHANNEL_NUM_ / _GROUP_ + _IN_CHANNEL_NUM_ / _GROUP_; a++) {//input kernel loop
                    conv_kernel_general_buf(in_data3D[a],
                                          kernel_weights[b * _IN_CHANNEL_NUM_ / _GROUP_ + a % (_IN_CHANNEL_NUM_ / _GROUP_)],
                            //kernel_weights[b * _IN_CHANNEL_NUM_ + a],
                                          out_data3D[b]);
                }
