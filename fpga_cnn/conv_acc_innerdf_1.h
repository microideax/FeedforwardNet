...
//        #pragma HLS PIPELINE
    ...
    // Load input data
    void in_buf_load(T buf[][(Tr-1)*S_max + K_max][(Tc-1)*S_max + K_max], T *in_data, int in_offset, int n, int r, int c, int S, int K, int P, int R_IN, int C_IN, int N) {
        ...
        for(int i=0; i<K; i++){
            for(int j=0; j<K; j++){
                for(int tr=0; tr<Tr; tr++){
                    if(R_OUT < r+Tr && tr+r == R_OUT){
                        break;
                    }
                    for(int tc=0; tc<Tc; tc++){
#pragma HLS PIPELINE
                        if(C_OUT < c+Tc && tc+c == C_OUT){
                            break;
                        }
...
#endif
                        if (act) {
                            *(out_data + out_offset + i * R_OUT * C_OUT + j * C_OUT + k) = relu(out_buf[i - m][j - r][k - c]);
                        }
                        else {
                            *(out_data + out_offset + i * R_OUT * C_OUT + j * C_OUT + k) = out_buf[i - m][j - r][k - c];
                        }
                    }
                }
            }
...
#pragma HLS DATAFLOW
   ...
   //--------------------------Load input B W D in ping-pong manner-------------------------//
                        ...
                        // load input data
                        in_buf_load(in_buf_1, in_data, in_offset, n, r, c, S, K, P, R_IN, C_IN, N);
                        // load input weights
            ...
            for (int i = 0; i < M; i++) {
                for (int j = 0; j < R_OUT; j++) {
                    for(int k = 0; k < C_OUT; k++){
                        conv_out << *(out_data + out_offset + i*R_OUT*C_OUT + j*C_OUT + k) << " ";
                    }
                    conv_out << endl;
                }
                conv_out << endl;
