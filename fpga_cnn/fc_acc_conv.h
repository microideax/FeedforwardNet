...
#pragma HLS UNROLL
                                if (i == 0 && j == 0 && tn == 0 && n == 0)
                                    out_buf[tm][tr][tc] = b_buf[tm] + w_buf[tn][tm][i + w_offset][j] * in_buf[tn][S * (tr) + i + i_offset][S * (tc) + j];
                                else
                                    out_buf[tm][tr][tc] = out_buf[tm][tr][tc] + w_buf[tn][tm][i + w_offset][j] * in_buf[tn][S * (tr) + i + i_offset][S * (tc) + j];
                            }
                        }
                    }
                }
            ...
            for (int c = 0; c < C_OUT; c += Tc)
            {
                for (int m = 0; m < M; m += Tm)
                {
                    for (int n = 0; n < N; n += 2 * Tn)
                    {
                        ...
                        //------------------------------compute buffered data -----------------------------------//
                        conv_engine(in_buf_0, w_buf_0, b_buf_0, out_buf_0, S, n, r, c, K, R_OUT, C_OUT, 0, 0);
                        conv_engine(in_buf_0, w_buf_0, b_buf_0, out_buf_0, S, n + Tn, r, c, K, R_OUT, C_OUT, 0, 0);
                        //---------------------------transfer output data----------------------------------------//
                        output_res(out_buf_0, out_data_1, out_offset, n, m, r, c, N, M, R_OUT, C_OUT, act);
                        output_res(out_buf_0, out_data_1, out_offset, n + Tn, m, r, c, N, M, R_OUT, C_OUT, act);
                    }
