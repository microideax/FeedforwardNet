...
template <typename T, typename W, typename G, int pTn, int pTr, int pTc, int pS_max, int pK_max>
class max_pool_acc {
...
public:
	...
	// Reset output buffer
	void out_buf_reset(G buf[][pTr][pTc]){
        for(int i = 0; i < pTn; i++){
            for(int j = 0; j < pTr; j++){
                for(int k = 0; k < pTc; k++){
                    buf[i][j][k] = G(0);
    ...
    // Load input data
    void in_buf_load(T buf[][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max],T *in_data_1, int n, int r, int c, int S, int K, int P, int R, int C, int N, int R_IN, int C_IN, int TR, int TC) {
...
#pragma HLS UNROLL
    ...
    void pool_engine(T in_buf[][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max], G out_buf[][pTr][pTc], int S, int n, int r, int c, int K, int R, int C, int TR, int TC) {
        if (n >= 0) {
            for (int i = 0; i < K; i++) {
                for (int j = 0; j < K; j++) {
...
#pragma HLS UNROLL
    ...
    void output_res(G out_buf[][pTr][pTc],G *out_data_1, int n, int r, int c, int N, int R, int C, bool act) {
        if (n >= 0) {
...
#pragma HLS PIPELINE
    ...
    void in_and_com(T* in_data_1,
                    T in_buf_0[][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max],
                    T in_buf_1[][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max],
                    G out_buf_0[][pTr][pTc],
                    bool i_buf_ptr,
                    int r,
                    int c,
...
///////////////////////------------------conv accelerator----------------//////////////////////////
    void max_pool_layer_dbuf(
        ...
        int TR=0;
        int TC=0;
        T in_buf_1[pTn][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max];
        T in_buf_0[pTn][(pTr-1)*pS_max + pK_max][(pTc-1)*pS_max + pK_max];
        G out_buf_1[pTn][pTr][pTc];
        G out_buf_0[pTn][pTr][pTc];
...
/*
        for(int r = 0; r < R; r += pTr){
            for(int c = 0; c < C; c += pTc){
                if (pool_regs.o_flag == 0) {
                    output(out_data_1, out_buf_0, r, c, N, R, C, act);
                } else {
                    output(out_data_1, out_buf_1, r, c, N, R, C, act);
                }
            }
        }
*/
