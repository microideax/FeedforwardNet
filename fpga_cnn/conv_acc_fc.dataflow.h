#ifndef _CONV_ACC_FC_H_
#define _CONV_ACC_FC_H_
...
public:
	conv_acc_fc() : conv_layer_number(0) {conv_layer_number = 0;};
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Starting conv_acc_innerdf_fc layer ...." << endl;
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Finished conv_acc_innerdf_fc layer ...." << endl;
            ofstream conv_out;
            conv_out.open("fc_out_data.txt",ios::app);
            conv_out <<"fc output: "<< endl;
            for (int i = 0; i < M/1; i++) {
