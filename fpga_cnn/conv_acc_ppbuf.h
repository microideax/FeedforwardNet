...
#pragma HLS UNROLL
    ...
    void output_res(G out_buf[][Tr][Tc], G *out_data, int out_offset, int m, int r, int c, int M, int R, int C){
        for(int i = m; i < m+Tm; i++){
            if(M < m+Tm && i == M){
                break;
            }
...
//            out_buf_reset(out_buf_2);
   ...
   //--------------------------Load input B W D in ping-pong manner-------------------------//
                        ...
                        while (buf_1_full == false || buf_2_full == false) {
                            cout << "start loading buffer--------------------------" << endl;
                            if (loadbufPtr == 0) {
                                T (*act_in_buf)[(Tr - 1) * 6 + 11][(Tc - 1) * 6 + 11] = in_buf_1;
                                W (*act_w_buf)[Tm][11][11] = w_buf_1;
                                W *act_b_buf = b_buf_1;
                                buf_1_act = true;
                                cout << "set load buffer 1" << endl;
                            } else {
                                T (*act_in_buf)[(Tr - 1) * 6 + 11][(Tc - 1) * 6 + 11] = in_buf_2;
                                W (*act_w_buf)[Tm][11][11] = w_buf_2;
                                W *act_b_buf = b_buf_2;
                                buf_2_act = true;
                                cout << "set load buffer 2" << endl;
                            }
                            //load input bias
                            b_buf_load(act_b_buf, layer_bias, bias_offset, m);
                            // load input data
                            in_buf_load(act_in_buf, in_data, in_offset, n, r, c, S, K, P, R, C, N);
                            // load input weights
                            w_buf_load(act_w_buf, layer_weights, weight_offset, n, m, K, N, M);
                            if (loadbufPtr == 0) {
                                ...
                                cout << "buffer 2 full" << endl;
                                buf_2_full = true;
                                loadbufPtr = 0;
                            }
                            cout << "buffer loading status, buffer 1 = "<< buf_1_full << "  buffer 2 = " << buf_2_full << endl;
//                            counter++;
//                            if (counter == 10) break;
                        }

  //------------------------------compute buffered data -----------------------------------//
                        while (buf_1_full == true || buf_2_full == true) {
                            ...
                            if (combufPtr == 0) {
                                ...
                                W *com_b_buf = b_buf_1;
                                buf_1_act = false;
                                cout << "set compute buffer 1" << endl;
                            } else {
                                T (*com_in_buf)[(Tr - 1) * 6 + 11][(Tc - 1) * 6 + 11] = in_buf_2;
                                W (*com_w_buf)[Tm][11][11] = w_buf_2;
                                W *com_b_buf = b_buf_2;
                                buf_2_act = false;
                                cout << "set compute buffer 2" << endl;
                            }

                            conv_engine(com_in_buf, com_w_buf, com_b_buf, out_buf, S, n, r, c, K, R, C);
...
//                            if (counter == 10) break;
                    ...
                    output_res(out_buf, out_data, out_offset, m, r, c, M, R, C);
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
