#ifndef _CONV_ACC_W_BN_H_
#define _CONV_ACC_W_BN_H_
...
public:
	conv_acc_w_bn() : conv_layer_number(0) {conv_layer_number = 0;};
...
#pragma HLS UNROLL
                   	if ((n + Tn > N && i + 0 >= N ) || j < 0 || j >= R_IN || k < 0 || k >= C_IN) {
                       	buf[i + 0 - n][j - r * S + P][k - c * S + P] = T(0);
                   	} else {
                       	buf[i + 0 - n][j - r * S + P][k - c * S + P] = *(in_data_1 + in_offset + i/1 * R_IN * C_IN + j * C_IN + k);
               		}
				}
			}
		}
...
#pragma HLS UNROLL
    ...
    void conv_engine(T in_buf[][(Tr-1)*S_max + K_max][(Tc-1)*S_max + K_max], W w_buf[][Tm][K_max][K_max], G out_buf[][Tr][Tc], int S, int n, int r, int c, int K, int R_OUT, int C_OUT){
        for(int i=0; i<K; i++){
            for(int j=0; j<K; j++){
...
#pragma HLS UNROLL
                                if(i==0&&j==0&&tn==0&&n==0)
                                    out_buf[tm][tr][tc] = w_buf[tn][tm][i][j]*in_buf[tn][S*(tr)+i][S*(tc)+j];
                                else
    ...
    void output_res(G out_buf[][Tr][Tc],G *out_data_1, int out_offset, int n, int m, int r, int c, int N, int M, int R_OUT, int C_OUT, bool act
        , W *bn_mean, W *bn_denominator, int bn_offset
#if _SCALE_
        , W *scale_gamma, W *scale_beta, int scale_offset
#endif
        ){
        if (n >= N - Tn) {
...
#pragma HLS PIPELINE
                    for (int i = 0; i < Tm && i < M-m; i += 1) {
                        out_buf[i + 0][j - r][k - c] = 
                        (out_buf[i + 0][j - r][k - c] - *(bn_mean + bn_offset + i + m + 0)) * (*(bn_denominator + bn_offset + i + m + 0));
#if _SCALE_
                        out_buf[i + 0][j - r][k - c] = 
                        out_buf[i + 0][j - r][k - c] * (*(scale_gamma + scale_offset + i + m + 0)) + (*(scale_beta + scale_offset + i + m + 0));
#endif
                        if (act) {
                        	if (i + 0 < M-m)
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Starting conv_acc_innerdf_w_bn layer ...." << endl;
...
#pragma HLS DATAFLOW
  ...
  //------------------------------compute buffered data -----------------------------------//
                        conv_engine(in_buf_1, w_buf_1, out_buf_1, S, n, r, c, K, R_OUT, C_OUT);
  //---------------------------transfer output data----------------------------------------//
                        // transfer output data
                        output_res(out_buf_1, out_data_1, out_offset, n, m, r, c, N, M, R_OUT, C_OUT, act 
                            ,bn_mean, bn_denominator, bn_offset
#if _SCALE_
                            ,scale_gamma, scale_beta, scale_offset
#endif
                            );
                    }
                }
...
#if _C_DEBUG_MODE_
#if _KERNEL_DEBUG_
            cout << "Finished conv_acc_innerdf_w_bn layer ...." << endl;
            ofstream conv_out;
            conv_out.open("conv_w_bn_out_data.txt",ios::app);
            conv_out <<"conv_w_bn output: "<< endl;
            for (int i = 0; i < M/1; i++) {
